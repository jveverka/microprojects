...
public class Router {
    ...
    public RouterFunction<ServerResponse> route(DataSeriesService dataSeriesService,
                                                DataRecordService dataRecordService, ContextConfig config) {
        return RouterFunctions
                // Data Series - APIs
                .route(GET(config.getPath("/services/series")).and(accept(APPLICATION_JSON)), request -> {
                ...
                .andRoute(POST(config.getPath("/services/series")).and(accept(APPLICATION_JSON)), request -> {
                ...
                .andRoute(DELETE(config.getPath("/services/series/{groupId}/{name}")).and(accept(APPLICATION_JSON)), request -> {
                    DataSeriesId id =
                ...
                .andRoute(GET(config.getPath( "/services/series/{groupId}/{name}")).and(accept(APPLICATION_JSON)), request -> {
                    DataSeriesId id =
                            new DataSeriesId(request.pathVariable(GROUP_ID), request.pathVariable(NAME));
                ...
                // Data Records - APIs
                .andRoute(GET(config.getPath("/services/records/{groupId}/{name}")).and(accept(APPLICATION_JSON)), request -> {
                    DataSeriesId id =
                            new DataSeriesId(request.pathVariable(GROUP_ID), request.pathVariable(NAME));
                    Flux<DataRecord> dataRecordFlux = dataRecordService.getAll(id);
                    return ServerResponse.ok().body(dataRecordFlux, DataRecord.class);
                })
                .andRoute(GET(config.getPath("/services/records/{groupId}/{name}/{startTime}/{duration}")).and(accept(APPLICATION_JSON)), request -> {
                    DataSeriesId id =
                            new DataSeriesId(request.pathVariable(GROUP_ID), request.pathVariable(NAME));
                ...
                .andRoute(POST(config.getPath("/services/records")).and(accept(APPLICATION_JSON)), request -> {
                ...
                .andRoute(PUT(config.getPath("/services/records")).and(accept(APPLICATION_JSON)), request -> {
                    Mono<InsertDataRecord> monoBody = request.bodyToMono(InsertDataRecord.class);
                    Mono<GenericResponse> genericResponseMono = monoBody.flatMap(dataRecordService::save);
                    return ServerResponse.ok().body(genericResponseMono, GenericResponse.class);
                })
                .andRoute(DELETE(config.getPath("/services/records/{groupId}/{name}/{timeStamp}")).and(accept(APPLICATION_JSON)), request -> {
                    DataSeriesId id =
                            new DataSeriesId(request.pathVariable(GROUP_ID), request.pathVariable(NAME));
